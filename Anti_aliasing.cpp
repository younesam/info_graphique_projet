...
#pragma omp parallel for
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			for (int k = 0; k < nrays; k++) {
				double r1 = uniform(engine);
				double r2 = uniform(engine);
				double r2s = sqrt(-2 * log(r2));
				Vector u(j - W / 2 + 0.5 + cos(2 * M_PI * r1) * r2s, -i + H / 2 - 0.5 + sin(2 * M_PI * r1) * r2s, -W / fov);
				u.normalize();
				Ray r(O, u);
				...
				lumiere_couleur += getColor(r, scene, 8, Source);
			}
	...
	std::cout << "Temps d'execution : " << time << " secondes" << std::endl;
