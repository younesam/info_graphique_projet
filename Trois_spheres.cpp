...
#pragma omp parallel for
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			//	visibilite = 0;
			//};
			double cals = dot(N, omega);
			double lumiere = intensite * max(dot(N, omega), 0.) / (4 * sqr(M_PI * d));
			Vector lumiere_couleur = first_albedo * lumiere;
			//printf(intersection);

			// Correction Gamma
			double gamma = 2.2;

			image[(i * W + j) * 3 + 0] = first_intersect ? min(lumiere_couleur[0], 255.) : 0;   // RED
			image[(i * W + j) * 3 + 1] = first_intersect ? min(lumiere_couleur[1], 255.) : 0;  // GREEN
			image[(i * W + j) * 3 + 2] = first_intersect ? min(lumiere_couleur[2], 255.) : 0;  // BLUE
		}
	}
